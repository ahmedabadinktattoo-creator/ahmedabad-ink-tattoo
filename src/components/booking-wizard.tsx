...
export function BookingWizard({ initialArtist = "" }: { initialArtist?: string }) {
  ...
  async function submit(form: HTMLFormElement) {
    ...
    try {
      ...
      if (result.demo && result.reference) { setConfirmation({ reference: result.reference, demo: true }); return; }
  ...
  return (
    <form className="booking-wizard" onSubmit={(event) => { event.preventDefault(); void submit(event.currentTarget); }}>
      ...
      <div className="wizard-stage">
        ...
        <fieldset hidden={step !== 2}><legend>Show us the feeling.</legend><p className="field-intro">Upload one optional visual reference. We use it to understand direction—not to copy another artist’s work.</p><label className="upload-field"><input name="reference" type="file" accept="image/jpeg,image/png,image/webp" /><span className="upload-icon">↑</span><strong>Choose a reference image</strong><small>JPG, PNG or WebP · Up to 8 MB</small></label></fieldset>
