...
export default async function ArtistPage({ params }: Props) {
  ...
  return (
    <div className="artist-profile">
      ...
      <section className="profile-about section"><div className="section-index">About / {artist.name}</div><div><h2>Work with<br /><em>meaning.</em></h2><p>{artist.bio}</p><div className="button-row"><Link className="button gold" href={`/book?artist=${artist.slug}`}>Book with {artist.name}</Link><a className="text-link" href={artist.instagram} target="_blank" rel="noreferrer">Instagram ↗</a></div></div></section>
