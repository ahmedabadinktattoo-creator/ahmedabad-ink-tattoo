...
function studioEmailHtml(booking: Confirmation) {
  return `<div style="font-family:Arial,sans-serif;color:#181714;line-height:1.6"><h1>New confirmed booking</h1>${details(booking)}<h2>Customer</h2><p>${escapeHtml(booking.name)}<br><a href="mailto:${escapeHtml(booking.email)}">${escapeHtml(booking.email)}</a><br><a href="https://wa.me/${booking.phone.replace(/\D/g, "")}">${escapeHtml(booking.phone)}</a></p><h2>Tattoo brief</h2><p><strong>Placement:</strong> ${escapeHtml(booking.placement)}<br><strong>Size:</strong> ${escapeHtml(booking.size)}</p><p>${escapeHtml(booking.idea)}</p><p><a href="https://www.ahmedabadinktattoo.com/admin#bookings">Open admin dashboard</a></p></div>`;
}
